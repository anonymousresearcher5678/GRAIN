...
asserts below verify that before doing so. The CSV is the source of truth.

`--anchor keep` (the default) preserves the CSV's own month-day anchor, which
`first_fraction` in `blgt_2.Rmd` sets (1 = Jan-1, 0 = Dec-31; the shipped
checkpoint was trained on Dec-31). `--anchor random` draws an independent
month-day per series so the calendar embeddings see more than one date; on a
yearly grid `y` does not depend on the anchor, so no regeneration is needed.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=("keep", "random"),
        default="keep",
        help="keep: use the CSV's month-day anchor (set by first_fraction in "
             "blgt_2.Rmd). random: draw an independent anchor per series so the "
