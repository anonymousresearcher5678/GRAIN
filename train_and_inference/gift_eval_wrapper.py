"""GIFT-Eval adapter for the yearly GRAIN.
...
for `gluonts.model.evaluation.evaluate_forecasts`. The model's predictor is
already a real `PyTorchPredictor`, so nothing here re-implements padding or
windowing -- the estimator's own transformation chain is reused. Four things do
have to change, and each is a silent-wrong-answer bug if
it is missed:
...
2. CALENDAR FEATURES FROM A PERIOD, NOT A TIMESTAMP. Entries carry no dates.
   `Period.to_timestamp()` overflows on M4 yearly --
...
# Targets past this are treated as infinite and clamped.
INF_CLAMP = 1e10
...
class CastTarget(SimpleTransformation):
    ...
    Infinities have no such path, so they are clamped to +/-INF_CLAMP.
    """
...
class GiftForecastNet(torch.nn.Module):
    ...
    def __init__(
        ...
        # One column per forecast key. "mean" takes the median: the head is a
        # pinball head and has no mean output.
        cols = []
