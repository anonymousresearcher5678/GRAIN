...
synthetic_m3_yearly <- generate_synthetic_dataset_fixed(
  data = m3_yearly_train_data,
  n_synthetic = 200, #length
  n_scenarios = 250, # number of different series
  frequency = 1,
  parallel = TRUE,  # Use parallel for speed
  n_cores = 5,
  first_fraction = 0 # 1 all start of year 0.5 half half 0 all end of year 
)
...
# Plot series
for (i in 1) {
  series_name <- unique(m3_yearly_train_data$series_name)[i]
...
```{r}
library(ggplot2)
series_name <- unique(m3_yearly_train_data$series_name)[1]
...
m3_yearly_synth_data <- m3_yearly_syn %>%
  select(
...
```{r}
write.csv(m3_yearly_synth_data, "m3_yearly_200.csv")
